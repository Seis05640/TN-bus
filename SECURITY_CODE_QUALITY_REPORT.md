...
| --- | --- | --- | --- |
| **High** | **Secrets committed to source control** | Real Google Maps and OpenWeather API keys are committed in `.env`. Anyone with repo access (or any future leak) can abuse these paid services and the keys can no longer be trusted. | Remove `.env` from version control, rotate all exposed keys immediately, and inject secrets via environment variables/secret managers. Provide `.env.example` with placeholders instead. |
